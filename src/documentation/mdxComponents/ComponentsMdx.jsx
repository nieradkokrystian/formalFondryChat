    ...
    <ul className={`list-disc ${className} list-decimal mb-10`}>
      {Items?.map((item, index) => (
        <li key={index} className={`flex items-center gap-2 mt-2 max-w-3/4`}>
          ...
          <Span defaultColor={defaultColor} isLink={isLink} path={path}>
            {item.title}
          </Span>
          {item.description && (
...
export const H2 = ({ text, className }) => {
  return (
    <h2 className={`text-3xl font-bold text-gray-600 mb-10 ${className}`}>
      {text}
    </h2>
  );
};
...
export const NextButton = ({ children }) => {
  return (
    <Link
      ...
      className="nav-buttons text-white  bg-custom-30 px-4 py-2 rounded-md hover:bg-gray-600 p-4"
    >
      {children}
    </Link>
  );
...
export const PrevButton = ({ children }) => {
  return (
    <Link
      to="/docs/getting-started"
      className="nav-buttons bg-blue-500  text-white px-4 py-2 rounded-md hover:bg-gray-600 p-4"
    >
      {children}
    </Link>
  );
  ...
  return (
    <div className="p-4 mt-10  border-t-2 border-gray-400 bg-white items-center gap-10 w-full flex flex-row h-100">
      <div className="column flex flex-col items-start  h-3/5 ">
        <h1 className="text-gray-600 text-lg mb-3 flex w-fit gap-1">
          <Logo />
        </h1>
        <p className="text-gray-500 mt-1">Formal Foundry, LLC. </p>
        <p className="text-gray-500 mt-1">218 NW 24th Street, 2nd Floor</p>
        <p className="text-gray-500 mt-1">Miami FL 33127</p>
        <Link className="text-custom-30" to={``}>
      ...
      <div className="column flex flex-col items-start  h-3/5 ">
        ...
        <ul className="text-gray-500">
          <li className="mt-1 hover:bg-purple-50 hover:text-purple-600">
      ...
      <div className="column flex flex-col items-start h-3/5">
        ...
        <div className="socials flex flew-row mt-3 justify-start gap-2">
          <IconContext.Provider
            value={{ color: "purple", className: "text-2xl " }}
          >
            <Link
              className=" aspect-square rounded-full p-3 border-1 border-purple-300 bg-purple-200 flex justify-center items-center hover:bg-purple-50 "
        ...
        <button
          className="brave-button mt-4 text-gray-300"
          onClick={() => {
            setOn(!on);
          }}
        >
        ...
        {on && (
          <h1 className="text-sm text-gray-300">
            {" "}
            Shameless plug? <br /> Yes it is! <br />{" "}
            <Link
              target="_blank"
              className="text-purple-500"
              about="_blank"
              to={`https://github.com/exelenze`}
            >
              {" "}
              Find the brave Intern.
            </Link>{" "}
          </h1>
        )}
      </div>
